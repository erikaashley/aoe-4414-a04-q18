...
#  
# Parameters:
# inputs date year, month, day, hour, minute, second and ECI position eci_x_km eci_y_km eci_z_km in km
# Output:
# outputs ecef coordinates ecef_x_km ecef_y_km ecef_z_km in km 
#
...
eci_x_km=float('nan') # ECI x position in km
eci_y_km=float('nan') # ECI y position in km
eci_z_km=float('nan') # ECI z position in km
...
eci_vect=[eci_x_km,eci_y_km,eci_z_km]
zrotat=[[math.cos(gmstrad),-math.sin(gmstrad),0],[math.sin(gmstrad),math.cos(gmstrad),0],[0,0,1]]

ecef_vect=[0,0,0]
for i in range(3):
    for j in range(3):
        ecef_vect[i]+=zrotat[i][j]*eci_vect[j]
